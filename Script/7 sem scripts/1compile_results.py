...
'''
This script compiles all student result data by mapping each subject code to the corresponding grade point 
achieved by every student. The output is then used to calculate the average GPA for each subject.

Grades (letter) are converted to numeric GPA scores as part of the process.
'''

# ─── CONFIG ──────────────────────────────────────────────────────

# Map letter grades → numeric score
GRADE_MAP = {
    'O': 10, 'A+': 9, 'A': 8, 'B+': 7,
    'B': 6, 'C': 5, 'D': 4, 'P': 4,
    'F': 0, 'Fail': 0, 'Absent': 0
